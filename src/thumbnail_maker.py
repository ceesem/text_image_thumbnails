...
try:
    FONT_DIRS = {'library': '/Library/Fonts/',
...
def build_fonts(font_name, font_location, size=18, title_size=20, title_index=2, author_index=1, regular_index=0):
...
def assemble_image(text_font_ctr_spacing, width, w_padding, v_padding, section_spacing):
    ...
    for line, fnt, is_centered, spacing, x, y in zip(lines_each, fnts_each, centered_each, spacing_each, x_pts, h_each):
        d.multiline_text((x, y), ''.join(line), font=fnt,
                         align=align_lookup[is_centered], fill=(0, 0, 0), spacing=spacing)
    return img
...
def thumbnail_image(title,
    ...
    section_spacing : int, jjkkoptional
    ...
    PIL.Image
    """
    title_font, author_font, abstract_font = build_fonts(font_name,
                                                         font_location,
                                                         size=font_size,
